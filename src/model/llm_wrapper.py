...
class QuantizedHFLLM(CustomLLM):
    ...
    def metadata(self) -> LLMMetadata:
        return LLMMetadata(
            ...
            num_output=self._metadata_num_output(),
            model_name=getattr(self.model.config, "_name_or_path", self.model.__class__.__name__),
            is_chat_model=False,
        )

    def _model_max_sequence_length(self) -> int | None:
        candidates = [
            getattr(self.model.config, "max_position_embeddings", None),
            getattr(self.model.config, "n_positions", None),
            getattr(self.model.config, "max_sequence_length", None),
            getattr(self.model.config, "seq_length", None),
            getattr(getattr(self.model, "generation_config", None), "max_length", None),
    ...
    def _metadata_num_output(self) -> int:
        if self.max_new_tokens is not None:
            return self.max_new_tokens
        model_limit = self._model_max_sequence_length()
        if model_limit is None:
            return self.context_window
        return max(model_limit - self.context_window, 1)

    def _resolve_generation_lengths(self, prompt_len: int, **kwargs: Any) -> tuple[int | None, int]:
        max_new_tokens = kwargs.get("max_new_tokens", self.max_new_tokens)
        if max_new_tokens is None:
            model_limit = self._model_max_sequence_length()
            if model_limit is not None:
                max_new_tokens = max(model_limit - prompt_len, 1)
    ...
    def _generate_ids(self, prompt: str, **kwargs: Any) -> torch.Tensor:
        ...
        max_new_tokens, min_new_tokens = self._resolve_generation_lengths(prompt_len, **kwargs)
